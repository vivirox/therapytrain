...
function addRateLimitHeaders(
  headers: Headers,
  rateLimitInfo: { remaining: number; reset: number; total: number },
) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!userId) {
      await logger.warn("Missing userId in request", {
        userId,
        threadId,
        mode,
      });
      ...
      try {
        const messages = await cache(
          ...
          async () => {
            ...
            // Building the query dynamically for TypeScript type safety
            const baseQuery = supabase
              .from("messages")
              .select("*")
              .order("timestamp", { ascending: true });

            // Apply filtering based on whether we have a threadId
            const query = threadId
              ? baseQuery.eq("thread_id", threadId)
              : baseQuery.or(
        ...
        await logger.info("Message history fetched", {
          userId,
          threadId,
        ...
        await logger.error("Error fetching message history", error as Error, {
          userId,
          threadId,
        });
    ...
    if (!threadId) {
      ...
      return new NextResponse("Missing threadId for SSE connection", {
        status: 400,
      });
    }
    ...
    const selectedInstance =
      await instanceManager.selectInstanceForConnection(userId);

    // If no healthy instances available
    if (!selectedInstance) {
      await logger.error("No healthy instances available", undefined, {
        userId,
      });
    ...
    await logger.error("Unexpected error in GET handler", typedError);
    await resourceMonitor.trackError(typedError);
    return new NextResponse("Internal Server Error", { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      await logger.warn("Unauthorized message send attempt", {
        recipientId,
        threadId,
        requestId,
      });
    ...
    await metrics.incrementCounter("rate_limit_check", {
    ...
    const response = new StreamingTextResponse(await streamToAsyncIterable(stream));
    addRateLimitHeaders(response.headers, rateLimitInfo);
    ...
    await Promise.all([
      metrics.incrementCounter("errors", {
        type: typedError.name || "unknown",
      }),
      logger.error("Chat error", typedError, {
        requestId,
        duration,
        stack: typedError.stack || "No stack trace",
      }),
      resourceMonitor.trackError(typedError),
    ]);
