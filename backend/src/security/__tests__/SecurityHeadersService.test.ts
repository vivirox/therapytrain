...
describe('SecurityHeadersService', () => {
    ...
    describe('Middleware', () => {
        it('should set all security headers by default', async () => {
            ...
            // Verify HSTS header
            expect(mockResponse.setHeader).toHaveBeenCalledWith('Strict-Transport-Security', expect.stringContaining('max-age=31536000'));
            // Verify CSP header
        ...
        it('should handle custom CSP directives', async () => {
            ...
            const customService = new SecurityHeadersService(mockSecurityAuditService, customConfig);
        ...
        it('should log violations when headers are missing', async () => {
            ...
            finishHandler();
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('SECURITY_HEADER_VIOLATION', 'HIGH', expect.objectContaining({
                headerName: 'CSP',
        ...
        it('should handle errors gracefully', async () => {
            ...
            const middleware = securityHeadersService.middleware();
            await middleware(mockRequest as Request, mockResponse as Response, nextFunction);
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('SECURITY_HEADERS_ERROR', 'HIGH', expect.objectContaining({
                error: 'Header error',
            }));
    ...
    describe('Nonce Management', () => {
        ...
        it('should cleanup old nonces', () => {
            const sessionId = 'test-session';
            const nonces = Array(15).fill(null).map(() => (securityHeadersService as unknown).generateNonce());
    ...
    describe('Violation Reporting', () => {
        it('should handle CSP violation reports', async () => {
            ...
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('CSP_VIOLATION', 'HIGH', expect.objectContaining({
                ...mockViolationReport,
            }));
        });
    });
