...
export class SecurityIncidentService {
    ...
    constructor(private readonly securityAuditService: SecurityAuditService, private readonly webAuthnService: WebAuthnService) {
    ...
    private async incrementCounter(map: Map<string, number>, key: string, threshold: number, incidentType: IncidentType): Promise<boolean> {
    ...
    public async handleRateLimit(ip: string): Promise<void> {
        const exceeded = await this.incrementCounter(this.rateLimitCounters, ip, SecurityIncidentService.RATE_LIMIT_THRESHOLD, IncidentType.RATE_LIMIT_EXCEEDED);
        if (exceeded) {
            this.blockedIps.add(ip);
            await this.securityAuditService.recordAlert('IP_BLOCKED', 'HIGH', { ip, reason: 'Rate limit exceeded' });
        }
    }
    public async handleBruteForceAttempt(ip: string, userId?: string): Promise<void> {
        const exceeded = await this.incrementCounter(this.bruteForceCounters, ip, SecurityIncidentService.BRUTE_FORCE_THRESHOLD, IncidentType.BRUTE_FORCE_ATTEMPT);
        if (exceeded) {
            this.blockedIps.add(ip);
            await this.securityAuditService.recordAlert('BRUTE_FORCE_DETECTED', 'HIGH', { ip, userId });
        }
    }
    public async handleSuspiciousActivity(ip: string, userId: string, details: Record<string, any>): Promise<void> {
        const exceeded = await this.incrementCounter(this.suspiciousActivityCounters, ip, SecurityIncidentService.SUSPICIOUS_ACTIVITY_THRESHOLD, IncidentType.SUSPICIOUS_ACTIVITY);
        if (exceeded) {
            await this.securityAuditService.recordAlert('SUSPICIOUS_ACTIVITY_DETECTED', 'HIGH', { ip, userId, ...details });
        }
    }
    ...
    public async handleIncident(incident: Omit<SecurityIncident, 'resolved'>): Promise<void> {
        ...
        // Log the incident
        await this.securityAuditService.recordAlert(incident.type, incident.severity, {
    ...
    private async handleWebAuthnViolation(incident: Omit<SecurityIncident, 'resolved'>): Promise<void> {
        ...
        if (incident.userId) {
            await this.securityAuditService.recordAlert('WEBAUTHN_VIOLATION_RESPONSE', 'HIGH', {
                userId: incident.userId,
    ...
    private async handleCspViolation(incident: Omit<SecurityIncident, 'resolved'>): Promise<void> {
        ...
        await this.securityAuditService.recordAlert('CSP_VIOLATION_RESPONSE', 'MEDIUM', {
            sourceIp: incident.sourceIp,
    ...
    private async handleDataLeakAttempt(incident: Omit<SecurityIncident, 'resolved'>): Promise<void> {
        ...
        await this.securityAuditService.recordAlert('DATA_LEAK_PREVENTED', 'HIGH', {
            sourceIp: incident.sourceIp,
            userId: incident.userId,
            details: incident.details
        });
    ...
    private async handleCriticalIncident(incident: SecurityIncident): Promise<void> {
        ...
        this.blockedIps.add(incident.sourceIp);
        await this.securityAuditService.recordAlert('CRITICAL_INCIDENT_RESPONSE', 'CRITICAL', {
            incident,
