...
export class AccountRecoveryService {
    ...
    async validateSecurityQuestionAnswers(
        storedQuestions: SecurityQuestion[],
        providedAnswers: { questionId: string; answer: string }[]
    ): Promise<boolean> {
        try {
            ...
            await this.securityAuditService.recordAlert(
                'SECURITY_ANSWER_VALIDATION_ERROR',
                'HIGH',
                {
                    error: error instanceof Error ? error.message : 'Unknown error'
                }
