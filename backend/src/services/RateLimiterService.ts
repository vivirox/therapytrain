...
export class RateLimiterService {
   ...
   */
  handleSuspiciousActivity = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    if (this.suspiciousList.has(ip)) {
      const customLimit =
        this.customLimits.get(ip) || this.DEFAULT_MAX_REQUESTS / 2;
      if (this.checkRateLimit(ip, customLimit, this.DEFAULT_WINDOW_MS)) {
        this.temporaryBlocks.set(ip, {
          until: Date.now() + this.DEFAULT_WINDOW_MS,
        });
  ...
  async decreaseLimit(ip: string, decreaseBy?: number): Promise<void> {
    ...
    const newLimit = decreaseBy
      ? currentLimit - decreaseBy
  ...
  /**
   * Check if a given key has exceeded its rate limit and tracks the request
   * @param key - Unique identifier (e.g., IP address, user ID)
  ...
  checkLimit(
    ...
    if (this.blacklistedIPs.has(key)) {
      this.securityAudit.recordEvent("RATE_LIMIT_BLOCKED", {
        key,
        action,
        reason: "blacklisted",
      });
    ...
    const tempBlock = this.temporaryBlocks.get(key);
    if (tempBlock && tempBlock.until > Date.now()) {
      this.securityAudit.recordEvent("RATE_LIMIT_BLOCKED", {
        key,
        action,
        reason: "temporary_block",
        expiresIn: Math.round((tempBlock.until - Date.now()) / 1000),
      });
      return true; // Rate limited
    }

    // Apply rate limiting
    return this.isRateLimited(key, action, maxRequests, windowMs);
  }
}
