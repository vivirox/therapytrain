...
export class SecureBackupService {
    ...
    constructor(private readonly securityAuditService: SecurityAuditService, private readonly hipaaAuditService: HIPAACompliantAuditService, private readonly keyRotationService: EncryptionKeyRotationService, backupPath?: string) {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            await this.securityAuditService.recordAlert('BACKUP_SERVICE_INIT_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            throw error;
        }
    }
    async createBackup(dataType: string, sourcePath: string, metadata?: Record<string, any>): Promise<BackupMetadata> {
        ...
        try {
            ...
            if (config.compressionRequired) {
                await pipeline(createReadStream(sourcePath), createGzip(), createWriteStream(tempFilePath));
            ...
            if (config.encryptionRequired) {
                ...
                await pipeline(createReadStream(tempFilePath), cipher, createWriteStream(backupFilePath));
            ...
            await this.securityAuditService.recordAlert('BACKUP_CREATE_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error',
                dataType
            });
            throw error;
        ...
        try {
            ...
            if (metadata.encryptionKeyId) {
                try {
                    ...
                    await pipeline(createReadStream(backupPath), decipher, createWriteStream(verificationPath));
            ...
            if (metadata.compressionRatio) {
                try {
                    ...
                    await pipeline(createReadStream(verificationPath), createGunzip(), createWriteStream(decompressedPath));
            ...
            await this.securityAuditService.recordAlert('BACKUP_VERIFY_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error',
                backupId
            });
            throw error;
    ...
    async testRestoration(backupId: string, targetPath: string): Promise<void> {
        try {
            ...
            if (metadata.encryptionKeyId) {
                const key = await this.keyRotationService.getKey(metadata.encryptionKeyId);
                const decipher = crypto.createDecipheriv(key.algorithm, key.keyMaterial, key.iv!);
                await pipeline(createReadStream(backupPath), decipher, createWriteStream(restorationPath));
            }
            else {
            ...
            if (metadata.compressionRatio) {
                ...
                await pipeline(createReadStream(restorationPath), createGunzip(), createWriteStream(decompressedPath));
            ...
            await this.securityAuditService.recordAlert('BACKUP_RESTORE_TEST_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error',
                backupId
            });
            throw error;
    ...
    private getBackupPath(metadata: BackupMetadata): string {
        return path.join(this.backupPath, metadata.dataType, `backup-${metadata.id}-${metadata.timestamp.toISOString()}.bak`);
    }
        ...
        for (const [dataType, config] of this.backupConfigs.entries()) {
            ...
            const timeout = setTimeout(async () => {
                try {
                    ...
                    await this.securityAuditService.recordAlert('BACKUP_SCHEDULE_ERROR', 'HIGH', {
                        error: error instanceof Error ? error.message : 'Unknown error',
                        dataType
                    });
                }
    ...
    async cleanup(): Promise<void> {
        try {
            ...
            await this.securityAuditService.recordAlert('BACKUP_SERVICE_CLEANUP_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            throw error;
        }
