...
describe('SecureBackupService', () => {
    ...
    beforeEach(async () => {
        ...
        tempDir = path.join(os.tmpdir(), 'backup-test-' + Math.random().toString(36).slice(2));
        testDataDir = path.join(tempDir, 'test-data');
        await fs.mkdir(tempDir, { recursive: true });
        await fs.mkdir(testDataDir, { recursive: true });
        backupService = new SecureBackupService(mockSecurityAuditService, mockHipaaAuditService, mockKeyRotationService, tempDir);
    ...
    describe('Initialization', () => {
        it('should create required directories', async () => {
            ...
            const [backupExists, metadataExists, tempExists] = await Promise.all([
                ...
                fs.stat(tempPath).then(() => true).catch(() => false)
            ]);
        ...
        it('should create data type directories', async () => {
            ...
            const [phiExists, auditExists, configExists] = await Promise.all([
                fs.stat(phiPath).then(() => true).catch(() => false),
                fs.stat(auditPath).then(() => true).catch(() => false),
                fs.stat(configPath).then(() => true).catch(() => false)
            ]);
        ...
        it('should handle initialization errors', async () => {
            ...
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('BACKUP_SERVICE_INIT_ERROR', 'HIGH', expect.any(Object));
        });
    });
    describe('Backup Creation', () => {
        it('should create encrypted and compressed backup', async () => {
            ...
            const backupPath = path.join(tempDir, 'PHI', `backup-${metadata.id}-${metadata.timestamp.toISOString()}.bak`);
        ...
        it('should handle backup creation errors', async () => {
            ...
            await expect(backupService.createBackup('PHI', 'nonexistent.txt')).rejects.toThrow();
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('BACKUP_CREATE_ERROR', 'HIGH', expect.any(Object));
        });
        it('should reject invalid data types', async () => {
            await expect(backupService.createBackup('INVALID_TYPE', 'test.txt')).rejects.toThrow('No backup configuration found');
        });
    });
    describe('Backup Verification', () => {
        ...
        it('should handle verification errors', async () => {
            jest.spyOn(fs, 'stat').mockRejectedValueOnce(new Error('File not found'));
            await expect(backupService.verifyBackup('nonexistent-id')).rejects.toThrow();
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('BACKUP_VERIFY_ERROR', 'HIGH', expect.any(Object));
        });
    });
    describe('Restoration Testing', () => {
        ...
        it('should handle restoration errors', async () => {
            jest.spyOn(fs, 'stat').mockRejectedValueOnce(new Error('File not found'));
            await expect(backupService.testRestoration('nonexistent-id', 'restored.txt')).rejects.toThrow();
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('BACKUP_RESTORE_TEST_ERROR', 'HIGH', expect.any(Object));
        });
        it('should detect size mismatches during restoration', async () => {
            ...
            const restorationPath = path.join(testDataDir, 'restored.txt');
            await expect(backupService.testRestoration(metadata.id, restorationPath)).rejects.toThrow('Restored file size mismatch');
        });
    });
    describe('Cleanup', () => {
        ...
        it('should handle cleanup errors', async () => {
            ...
            expect(mockSecurityAuditService.recordAlert).toHaveBeenCalledWith('BACKUP_SERVICE_CLEANUP_ERROR', 'HIGH', expect.any(Object));
        });
    });
});
