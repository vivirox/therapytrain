...
export class HIPAACompliantAuditService {
    ...
    constructor(private readonly securityAuditService: SecurityAuditService, private readonly verificationKeyService: VerificationKeyService, auditLogPath?: string) {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            await this.securityAuditService.recordAlert('HIPAA_AUDIT_INIT_ERROR', 'HIGH', {
    ...
    async logEvent(event: Omit<HIPAAAuditEvent, 'id' | 'metadata'>): Promise<string> {
        try {
            ...
            if (this.isHighRiskEvent(fullEvent)) {
                await this.securityAuditService.recordAlert('HIPAA_HIGH_RISK_EVENT', 'HIGH', {
                    eventId,
            ...
            await this.securityAuditService.recordAlert('HIPAA_AUDIT_LOG_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error',
    ...
    async archiveOldEvents(): Promise<void> {
        try {
            ...
            for (const file of logFiles) {
                if (!file.startsWith('hipaa-audit-'))
                    continue;
                ...
                for (const policy of this.retentionPolicies) {
                    ...
                    if (fileDate < archiveDate) {
                        ...
                        await this.securityAuditService.recordAlert('HIPAA_AUDIT_ARCHIVED', 'LOW', {
                            file,
            ...
            await this.securityAuditService.recordAlert('HIPAA_AUDIT_ARCHIVE_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            throw error;
        }
    ...
    private async rotateLogIfNeeded(): Promise<void> {
        try {
            ...
            await this.securityAuditService.recordAlert('HIPAA_AUDIT_ROTATION_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            throw error;
        }
    ...
    private async appendToLog(event: HIPAAAuditEvent): Promise<void> {
        ...
        const logPath = path.join(this.auditLogPath, this.currentLogFile);
        await fs.appendFile(logPath, `${JSON.stringify(encryptedEvent)}\n`, 'utf-8');
    }
    ...
    private async initializeLastEventHash(): Promise<void> {
        try {
            ...
            const content = await fs.readFile(path.join(this.auditLogPath, lastFile), 'utf-8');
            ...
            await this.securityAuditService.recordAlert('HIPAA_AUDIT_HASH_INIT_ERROR', 'HIGH', {
                error: error instanceof Error ? error.message : 'Unknown error'
            });
            throw error;
        }
    ...
    private async verifyEventChain(events: Array<HIPAAAuditEvent>): Promise<void> {
        for (let i = 1; i < events.length; i++) {
            ...
            if (currentEvent.metadata.previousEventHash !== previousEvent.metadata.hashValue) {
                await this.securityAuditService.recordAlert('HIPAA_AUDIT_CHAIN_BROKEN', 'CRITICAL', {
