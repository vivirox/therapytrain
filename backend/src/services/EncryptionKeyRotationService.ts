...
export class EncryptionKeyRotationService {
    ...
    private readonly rotationConfigs: Map<KeyPurpose, KeyRotationConfig> = new Map();
    ...
    private async generateNewKey(purpose: KeyPurpose): Promise<EncryptionKey> {
        ...
        try {
            ...
            const key: EncryptionKey = {
                ...
                iv,
                createdAt: new Date(),
                expiresAt: new Date(Date.now() + config.rotationPeriod * 24 * 60 * 60 * 1000),
    ...
    private async loadExistingKeys(): Promise<void> {
        try {
            ...
            for (const file of keyFiles) {
                ...
                if (key.status === KeyStatus.ACTIVE && new Date(key.expiresAt) > new Date()) {
