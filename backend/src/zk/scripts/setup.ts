...
async function main() {
    try {
        ...
        if (!fs.access(ptauPath)) {
            ...
            await snarkjs.powersOfTau.contribute(path.join(keysDir, "pot12_0.ptau"), path.join(keysDir, "pot12_1.ptau"), "First contribution", entropy1);
            await snarkjs.powersOfTau.contribute(path.join(keysDir, "pot12_1.ptau"), path.join(keysDir, "pot12_2.ptau"), "Second contribution", entropy2);
            await snarkjs.powersOfTau.beacon(path.join(keysDir, "pot12_2.ptau"), path.join(keysDir, "pot12_beacon.ptau"), "Final beacon", crypto.randomBytes(32), 10);
            await snarkjs.powersOfTau.preparePhase2(path.join(keysDir, "pot12_beacon.ptau"), ptauPath);
        }
        ...
        for (let i = 0; i < 3; i++) {
            ...
            await snarkjs.zKey.contribute(path.join(keysDir, `circuit_${i}.zkey`), path.join(keysDir, `circuit_${i + 1}.zkey`), `Contributor ${i + 1}`, entropy);
        }
        // Apply a random beacon
        await snarkjs.zKey.beacon(path.join(keysDir, "circuit_3.zkey"), zkeyPath, "Final Beacon", crypto.randomBytes(32), 10);
        ...
        await fs.writeFile(path.join(buildDir, `${circuitName}Verifier.sol`), verifierCode);
