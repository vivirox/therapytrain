...
export class AuditController {
    ...
    async getAuditLogs(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const logs = await this.securityAudit.getAuditLogs(query.startTime, query.endTime, {
            ...
            await this.securityAudit.logDataAccess(req.user?.id || 'unknown', 'audit_logs', 'READ', {
                filters: query,
    ...
    async getSecurityMetrics(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            await this.securityAudit.logDataAccess(req.user?.id || 'unknown', 'audit_metrics', 'READ', {
                timeRange: {
    ...
    async getSecurityAlerts(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            await this.securityAudit.logDataAccess(req.user?.id || 'unknown', 'security_alerts', 'READ', {
    ...
    async getRateLimitEvents(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            await this.securityAudit.logDataAccess(req.user?.id || 'unknown', 'rate_limit_events', 'READ', {
                ip,
                userId,
                resultCount: data.length
            });
