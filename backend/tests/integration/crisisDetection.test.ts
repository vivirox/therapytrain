...
describe('Crisis Detection Verification', () => {
    ...
    describe('Direct Crisis Keywords', () => {
        ...
        test.each(directCrisisMessages)(
            'should detect direct crisis message: $content',
            async ({ content, expectedTopics, minSentimentScore }) => {
    ...
    describe('Indirect Crisis Indicators', () => {
        ...
        test.each(indirectCrisisMessages)(
            'should detect indirect crisis message: $content',
            async ({ content, severity }) => {
                const response = await aiService.processMessage(userId, content);
                ...
                expect(response.metadata?.followUpQuestions.some(q =>
    ...
    describe('Crisis Escalation Patterns', () => {
        ...
        it('should maintain crisis state after initial detection', async () => {
            ...
            const initialResponse = await aiService.processMessage(
                userId,
    ...
    describe('False Positive Prevention', () => {
        ...
        test.each(nonCrisisMessages)(
            'should not trigger crisis detection for common expressions: %s',
            async (message) => {
                const response = await aiService.processMessage(userId, message);
                ...
                expect(securityAudit.recordEvent).not.toHaveBeenCalledWith(
                    'crisis_detected',
                    expect.any(Object)
                );
    ...
    describe('Response Time Requirements', () => {
        ...
        it('should handle multiple crisis messages concurrently', async () => {
            ...
            const start = performance.now();
            await Promise.all(
                crisisMessages.map(msg => aiService.processMessage(userId, msg))
            );
            const duration = performance.now() - start;
