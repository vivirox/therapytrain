...
const InterventionTracker: React.FC<InterventionTrackerProps> = ({
  sessionId,
  ...
  const getEffectivenessIcon = (score: number) => {
    ...
    if (score >= 3) return <AlertTriangle className="h-5 w-5" />;
    ...
    <div className="space-y-4">
      <Card className="p-4">
        ...
        <div className="mb-6">
          ...
          <Progress value={effectiveness.overall * 20} className="h-2" />
        </div>

        <div className="space-y-4">
          {Object.entries(effectiveness.byApproach).map(([approach, score]) => (
            <div key={approach}>
              ...
              <div className="flex justify-between items-center mt-2">
                <div className="flex gap-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onEffectivenessUpdate(intervention.type, 1)}
                    className={intervention.effectiveness <= 2 ? 'text-red-500' : ''}
                  >
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onEffectivenessUpdate(intervention.type, 5)}
                    className={intervention.effectiveness >= 4 ? 'text-green-500' : ''}
                  >
