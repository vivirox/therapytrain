...
describe("Chat API", () => {
  ...
  describe("GET handler", () => {
    ...
    it("should handle message history request", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/chat?userId=test-user&mode=history",
      );
      const response = await GET(req);
    ...
    it("should handle rate limit exceeded for history request", async () => {
      ...
      const mockRateLimitResponse = new Response("Rate limit exceeded", {
        status: 429,
      });
      (createRateLimitResponse as any).mockReturnValueOnce(
    ...
    it("should handle SSE connection request", async () => {
      ...
      (
        mockInstanceManager.selectInstanceForConnection as any
      ...
      const req = new NextRequest(
        "http://localhost/api/chat?userId=test-user&threadId=test-thread",
      );
      const response = await GET(req);
  ...
  describe("POST handler", () => {
    ...
    it("should handle message sending", async () => {
      ...
      (encryptMessageForRecipient as any).mockResolvedValueOnce(
      ...
      (StreamingTextResponse as any).mockImplementationOnce(
    ...
    it("should handle message storage error", async () => {
      ...
      mockSupabase
        .from()
        .insert.mockResolvedValueOnce({ error: new Error("Storage error") });
