...
class CrisisPrediction {
  ...
  static async assessRisk(
    ...
    // Fetch relevant data
    const [sessionData, clientHistory, recentMessages, behavioralPatterns] =
      await Promise.all([
    ...
    const behavioralFactors = await this.analyzeBehavioralRisk(
      recentMessages,
      behavioralPatterns,
    );
    const emotionalFactors = await this.analyzeEmotionalRisk(
      recentMessages,
      sessionData,
    );
    const contextualFactors = await this.analyzeContextualRisk(
      clientHistory,
      sessionData,
    );
    ...
    const escalationProtocol =
      urgency === "high" || urgency === "critical"
        ? this.getEscalationProtocol(urgency, overallRisk)
        : undefined;
    return {
      overallRisk,
      urgency,
  ...
  static async configureAlerts(
    ...
    const { error } = await dataService.upsertAlertConfig({
      client_id: clientId,
  ...
  static async triggerAlert(
    ...
    const { data: config, error } = await dataService.getAlertConfig(clientId);
    if (error) {
  ...
  private static async getSessionData(sessionId: string) {
    const { data, error } = await dataService.getSessionData(sessionId);
    if (error) {
  ...
  private static async getClientHistory(clientId: string) {
    const { data, error } = await dataService.getClientHistory(clientId);
    if (error) {
  ...
  private static async getRecentMessages(clientId: string) {
    const { data, error } = await dataService.getRecentMessages(clientId);
    if (error) {
  ...
  private static async getBehavioralPatterns(clientId: string) {
    const { data, error } = await dataService.getBehavioralPatterns(clientId);
    if (error) {
  ...
  private static async analyzeBehavioralRisk(
    ...
    for (const indicator of this.riskIndicators.behavioral) {
      const matches = messages.filter((m: { content: string }) =>
  ...
  private static async analyzeEmotionalRisk(
    ...
    if (sessionData.sentiment_trends) {
      ...
      const avgSentiment =
        recentSentiments.reduce((a: number, b: number) => a + b, 0) /
  ...
  private static async analyzeContextualRisk(
    ...
    for (const indicator of this.riskIndicators.contextual) {
      if (
        sessionData.context_notes?.includes(indicator) ||
  ...
  private static calculateOverallRisk(factors: Array<RiskFactor>): number {
    ...
    const typeScores = Object.entries(weights).map(([type, weight]: any) => {
      ...
      const weightedScores = typeFactors.map(
        (f: any) => f.severity * f.confidence,
      );
  ...
  private static generateRecommendations(
    ...
    switch (urgency) {
      ...
      case "high":
        recommendations.push(
          "Schedule immediate follow-up session",
    ...
    factors.forEach((factor: any) => {
      if (factor.severity > 0.7) {
        recommendations.push(
          `Address ${factor.type} concerns: ${factor.description}`,
        );
      }
    });
  ...
  private static determineNextSteps(
    ...
    switch (urgency) {
      case "critical":
        steps.push(
          "1. Contact emergency services if immediate danger",
          "2. Notify supervisor immediately",
          "3. Document all actions taken",
          "4. Implement crisis intervention protocol",
        );
        break;
      case "high":
        steps.push(
          "1. Schedule immediate follow-up session",
          "2. Contact support network",
          "3. Review and update safety plan",
      ...
      case "low":
        steps.push(
          "1. Maintain regular session schedule",
  ...
  private static getEscalationProtocol(
    urgency: "high" | "critical",
    uncertainty: number,
  ): string {
    const uncertaintyNote =
      uncertainty > 0.3
        ? "\n\nNOTE: High prediction uncertainty detected. Consider immediate comprehensive assessment."
        : "";
  ...
  private static async sendAlert(
    channel: AlertConfig["channels"][number],
    config: any,
    assessment: RiskAssessment,
  ): Promise<void> {
    const message =
  ...
  private static async sendInAppAlert(
    ...
    const { error } = await dataService.sendInAppAlert({
      recipients,
      message,
  ...
  private static async logAlert(
    ...
    const { error } = await dataService.logAlert({
      client_id: clientId,
      risk_level: assessment.urgency,
