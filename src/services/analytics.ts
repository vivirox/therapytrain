...
export class AnalyticsService {
  ...
  static async getSkillGrowth(userId: string, skillId: string): Promise<SkillGrowth> {
    try {
      const response = await fetch(
        `${this.GROWTH_ENDPOINT}/${userId}/skills/${skillId}`
      );
      if (!response.ok) throw new Error('Failed to fetch skill growth data');
      return await response.json();
    } catch (error) {
  ...
  static async generateLearningInsights(userId: string): Promise<LearningAnalytics> {
    try {
      ...
      const skillGrowthPromises = skills.map((skill: { id: string }) =>
  ...
  private static calculateLearningVelocity(
    ...
    const timeSpan =
      new Date(sortedTrends[sortedTrends.length - 1].date).getTime() -
      new Date(sortedTrends[0].date).getTime();
    const improvement =
      sortedTrends[sortedTrends.length - 1].value - sortedTrends[0].value;
  ...
  private static analyzeTimingPatterns(
    ...
    const hourlyAverages = new Array(24).fill(0).map((_: unknown, hour) => {
      ...
      return {
        hour,
        average:
          hourData.reduce((sum, d) => sum + d.value, 0) / (hourData.length || 1)
      };
    });
  ...
  private static analyzePerformancePatterns(
    ...
    if (performanceTrends.length < 2) return patterns;

    // Calculate overall trend
    const firstScore = performanceTrends[0].value;
    const lastScore =
      performanceTrends[performanceTrends.length - 1].value;
  ...
  private static analyzeEngagementPatterns(
    ...
    if (engagementTrends.length < 7) return patterns;
    ...
    const mostEngagedDay = Object.entries(weeklyEngagement).sort(
      ([, a], [, b]) => b - a
    )[0];

    patterns.push({
      pattern: 'Engagement Pattern',
      description: `You're most engaged on ${this.getDayName(
        Number(mostEngagedDay[0])
      )}s`,
      significance: 0.7
    });
  ...
  private static generateRecommendations(
    ...
    patterns: Array<{ pattern: string; description: string; significance: number }>
  ): string[] {
    ...
    if (metrics.completionRate < 0.7) {
      recommendations.push(
        'Consider breaking down learning sessions into smaller, more manageable chunks'
      );
    }
    ...
    if (metrics.averageScore < 0.8) {
      recommendations.push(
        'Review feedback from completed exercises to identify areas needing more focus'
      );
    }
    ...
    if (slowGrowthSkills.length > 0) {
      recommendations.push(
        'Consider seeking peer support or additional resources for challenging skills'
      );
    }
  ...
  static trackResourceEngagement(
    userId: string,
    resourceId: string,
    resourceType: string,
