...
export class ZKService {
    ...
    public async generateSessionProof(sessionData: SessionInput): Promise<ProofData> {
        try {
            ...
            await this.auditLogger.logProofEvent(sessionData.sessionId, 'N/A', 'GENERATE', 'failure', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    public async verifySessionProof(proofData: ProofData, publicInputs: {
        ...
        try {
            ...
            await this.auditLogger.logProofEvent(publicInputs.sessionId, 'N/A', 'VERIFY', isValid ? 'success' : 'failure', { publicSignals: publicInputs });
            return isValid;
        }
        catch (error) {
            await this.auditLogger.logProofEvent(publicInputs.sessionId, 'N/A', 'VERIFY', 'failure', { error: error.message });
            throw error;
        }
    }
