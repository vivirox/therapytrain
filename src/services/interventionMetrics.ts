...
class InterventionMetrics {
    ...
    private static calculateImmediateImpact(interventions: Array<any>): number {
        ...
        return interventions.reduce((sum: unknown, intervention: unknown) => {
            ...
            return sum + (immediateScore * weights.immediateResponse +
    ...
    private static calculateClientReceptiveness(interventions: Array<any>): number {
        return interventions.reduce((sum: unknown, intervention: unknown) => sum + intervention.client_engagement, 0) / (interventions.length || 1);
    }
    ...
    private static calculateOverallScore(components: Record<string, number>): number {
        ...
        return Object.entries(components).reduce((sum, [key, value]) => sum + value * weights[key as keyof typeof weights], 0);
    }
    private static analyzeTimeOfDayEffectiveness(interventions: Array<any>): Record<string, number> {
        ...
        return Object.entries(hourlyScores).reduce((acc, [hour, { sum, count }]) => ({
            ...acc,
            [hour]: sum / count
        }), {});
    }
    private static analyzeTypeEffectiveness(interventions: Array<any>): Record<string, number> {
        ...
        return Object.entries(typeScores).reduce((acc, [type, { sum, count }]) => ({
            ...acc,
            [type]: sum / count
        }), {});
    }
    private static analyzeContextEffectiveness(interventions: Array<any>): Record<string, number> {
        ...
        return Object.entries(contextScores).reduce((acc, [context, { sum, count }]) => ({
            ...acc,
            [context]: sum / count
        }), {});
    }
    private static calculateMetricImprovement(prevMetrics: any, nextMetrics: any): number {
        ...
        const improvements = metrics.map(metric => (nextMetrics[metric] - prevMetrics[metric]) / Math.abs(prevMetrics[metric] || 1));
