...
export class InterventionMetricsService {
    private readonly weights = {
        immediateResponse: 0.6,
        contextualRelevance: 0.4
    };

    static async trackIntervention(sessionId: string, type: string, context: Record<string, any>): Promise<string> {
        try {
    ...
    static async updateEffectiveness(interventionId: string, metrics: Partial<InterventionMetric>): Promise<void> {
        try {
    ...
    static async calculateEffectiveness(sessionId: string, timeframe: 'session' | 'day' | 'week' | 'month' = 'session'): Promise<EffectivenessScore> {
    ...
    private calculateImmediateImpact(interventions: Intervention[]): number {
        if (!interventions.length) {
            return 0;
        }

        const totalImpact = interventions.reduce((sum: number, intervention: Intervention) => {
            const immediateScore = (intervention.immediate_response + 1) / 2; // Convert -1:1 to 0:1
            const contextScore = intervention.contextual_relevance;
            return sum + (immediateScore * this.weights.immediateResponse +
                contextScore * this.weights.contextualRelevance);
        }, 0);

        return totalImpact / interventions.length;
    }

    private static async calculateSustainedEffect(interventions: Array<any>): Promise<number> {
        const followUpScores = await Promise.all(interventions.map(async (intervention: unknown) => {
        ...
        return followUpScores.reduce((sum: any, score: any) => sum + score, 0) /
            (followUpScores.length || 1);
    }

    private static calculateClientReceptiveness(interventions: Array<any>): number {
        return interventions.reduce((sum: number, intervention: unknown) => sum + intervention.client_engagement, 0) / (interventions.length || 1);
    }
    ...
    private static calculateImplementationSuccess(interventions: Array<any>): number {
        ...
        return interventions.reduce((sum: number, intervention: unknown) => {
            const followThroughScore = intervention.follow_through_rate;
            const adaptationScore = intervention.adaptation_score;
            return sum + (followThroughScore * weights.followThrough +
    ...
    private static calculateOverallScore(components: Record<string, number>): number {
        ...
        return Object.entries(components).reduce((sum: any, [key, value]: any) => sum + value * weights[key as keyof typeof weights], 0);
    }

    private static analyzeTimeOfDayEffectiveness(interventions: Intervention[]): Record<string, number> {
        const hourlyScores: Record<string, {
            sum: number;
            count: number;
        }> = {};
        ...
        return Object.entries(hourlyScores).reduce((acc: any, [hour, { sum, count }]: any) => ({
            ...acc,
            [hour]: sum / count
        }), {});
    }

    private static analyzeTypeEffectiveness(interventions: Intervention[]): Record<string, number> {
        const typeScores: Record<string, {
            sum: number;
            count: number;
        }> = {};
        interventions.forEach((intervention: any) => {
        ...
        return Object.entries(typeScores).reduce((acc: any, [type, { sum, count }]: any) => ({
            ...acc,
            [type]: sum / count
        }), {});
    }

    private static analyzeContextEffectiveness(interventions: Intervention[]): Record<string, number> {
        const contextScores: Record<string, {
            sum: number;
            count: number;
        }> = {};
        interventions.forEach((intervention: any) => {
        ...
        return Object.entries(contextScores).reduce((acc: any, [context, { sum, count }]: any) => ({
            ...acc,
            [context]: sum / count
        }), {});
    }

    private static calculateMetricImprovement(prevMetrics: any, nextMetrics: any): number {
        const metrics = ['sentiment', 'engagement', 'progress'];
        const improvements = metrics.map((metric: any) => (nextMetrics[metric] - prevMetrics[metric]) / Math.abs(prevMetrics[metric] || 1));
        return improvements.reduce((sum: any, imp: any) => sum + imp, 0) / metrics.length;
    }
    ...
    private static generateRecommendations(data: {
        ...
        if (data.overall < 0.4) {
            recommendations.push("Consider revising intervention strategies based on client feedback");
        }
        ...
        const bestHours = Object.entries(data.timeOfDayTrends)
            ...
            .slice(0, 2)
            .map(([hour]: any) => hour);
        recommendations.push(`Interventions are most effective during ${bestHours.join(":00 and ")}:00`);
        // Type-based recommendations
        const [bestType] = Object.entries(data.typeTrends)
            .sort(([, a], [, b]) => b - a);
        if (bestType) {
            recommendations.push(`"${bestType[0]}" interventions show highest effectiveness`);
        }
...
export default InterventionMetricsService;
