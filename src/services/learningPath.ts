...
export class LearningPathService {
  ...
  async generatePersonalizedPath(
    ...
    const curriculum = await this.aiAnalytics.generatePersonalizedCurriculum(
      userId,
      Object.keys(initialSkillLevels)
    );
  ...
  private calculateDifficulty(priority: number): LearningPathNode['difficulty'] {
    if (priority <= 0.3) return 'beginner';
    if (priority <= 0.7) return 'intermediate';
    return 'advanced';
  }
  ...
  private calculatePaceAdjustment(
    ...
    if (ratio > 0.7) return 'increase';
    if (ratio < 0.3) return 'decrease';
    return 'maintain';
  }
