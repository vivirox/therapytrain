...
export class ComplianceService {
  ...
  public async checkRetentionPolicy(session: any): Promise<{
    ...
    if (!this.validateRetentionSettings(retentionSettings)) {
      ...
      recommendations.push('Update data retention settings to meet compliance requirements');
    }
    ...
    if (!this.validateArchivalPolicies(archivalPolicies)) {
      ...
      recommendations.push('Update archival policies to meet compliance requirements');
    }
  ...
  private validateRetentionSettings(settings: any): boolean {
    ...
    if (!settings.retentionPeriod || typeof settings.retentionPeriod !== 'number') {
      return false;
    }
  ...
  private calculateTimingCompliance(schedule: any, logs: any[]): number {
    ...
    return timingScores.reduce((sum, score) => sum + score, 0) / timingScores.length;
  }
  ...
  private getNearestScheduleTime(schedule: any, date: Date): {
    hours: number;
    minutes: number;
  } {
