...
export class AuditService extends EventEmitter {
  ...
  private validateEvent(event: AuditEvent): void {
    ...
    if (!event.type || !event.action || !event.resourceType || !event.resourceId || !event.userId) {
    ...
    if (!event.type.split('.')[0] || !validEventTypes.includes(event.type.split('.')[0])) {
  ...
  public async generateAuditReport(
    ...
    try {
      ...
      const processedLogs = this.processLogsForReport(logs, options.detailLevel);
      ...
      switch (options.format) {
        ...
        case 'json':
        default:
          return this.generateJSONReport(processedLogs, options.includeMetadata);
      }
    } catch (error) {
  ...
  private generateJSONReport(logs: any[], includeMetadata: boolean = true): any {
    if (!includeMetadata) {
      return logs.map(log => {
  ...
  private generateCSVReport(logs: any[], includeMetadata: boolean = true): string {
    // Implement CSV generation logic
    return '';
  }

  private generatePDFReport(logs: any[], includeMetadata: boolean = true): Buffer {
