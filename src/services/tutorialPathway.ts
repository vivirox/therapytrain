...
export class TutorialPathwayService {
  ...
  private async generateRecommendedPath(
    userId: string,
    currentSkills: Record<string, number>,
    targetSkills: string[]
  ): Promise<string[]> {
    ...
    const relevantTutorials = Array.from(graph.entries())
      .filter(([_, node]) => 
        node.providedSkills.some(skill => skillGaps.includes(skill)) &&
        node.requiredSkills.every(skill => 
          currentSkills[skill] !== undefined && currentSkills[skill] >= 1
        )
      )
      .sort((a, b) => {
        // Sort by number of relevant skills provided
        const aRelevance = a[1].providedSkills.filter(skill => 
          skillGaps.includes(skill)
        ).length;
        const bRelevance = b[1].providedSkills.filter(skill => 
          skillGaps.includes(skill)
        ).length;
  ...
  public async updateLearningPathway(
    userId: string,
    targetSkills: string[]
    ...
    const currentSkills = Object.fromEntries(
      Object.entries(skillProgression.skills).map(([skill, data]: [string, any]) => [
        skill,
        data.level
      ])
    );
    ...
    const recommendedPath = await this.generateRecommendedPath(
      userId,
      currentSkills,
      targetSkills
    );
    ...
    const completedNodes = Object.values(skillProgression.skills).flatMap(
      (skill: any) => skill.completedTutorials
    );
