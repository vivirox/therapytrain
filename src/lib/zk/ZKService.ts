...
export class ZKService {
  ...
  public async getOrCreateSessionKeys(threadId: string): Promise<{ id: string; publicKey: string; privateKey: string }> {
  ...
  private async checkAndRotateKeys(threadId: string): Promise<void> {
    ...
    const shouldRotate = 
      ...
      Date.now() - metadata.lastRotation.getTime() >= SESSION_KEY_EXPIRY - KEY_TRANSITION_PERIOD;
  ...
  private async acquireRotationLock(threadId: string): Promise<boolean> {
    ...
    if (metadata.rotationLockExpiry && metadata.rotationLockExpiry > new Date()) {
      return false;
    }
  ...
  private async rotateSessionKeys(threadId: string): Promise<{ id: string; publicKey: string; privateKey: string }> {
    let retryCount = 0;
  ...
  public async getSessionKeys(threadId: string): Promise<{ id: string; publicKey: string; privateKey: string } | null> {
  ...
  public async encryptMessageWithSessionKey(
    ...
    try {
      ...
      await this.forwardSecrecy.initializeRatchet(metadata.threadId, sharedSecret, isInitiator);
