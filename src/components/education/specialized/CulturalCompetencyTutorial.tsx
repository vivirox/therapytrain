...
export const CulturalCompetencyTutorial: React.FC<CulturalCompetencyTutorialProps> = ({
  userId,
  caseStudyId,
  onComplete
}) => {
  ...
  useEffect(() => {
    if (!caseStudy || selectedOptions.length === 0) return;

    const generateFeedback = async () => {
      try {
        const adaptiveFeedback = await AIAnalyticsService.generateAdaptiveFeedback(
          userId,
          {
            metrics,
            selectedOptions,
  ...
  const handleComplete = async () => {
    ...
    try {
      const skillAnalysis = await AIAnalyticsService.analyzeCognitiveDevelopment(
        userId,
        results
      );
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Progress value={metrics.awareness} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between mb-1">
            ...
            <div>
              ...
              <Progress value={metrics.skills} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between mb-1">
              ...
              <Progress value={metrics.rapport} className="h-2" />
            </div>
          </div>
        </Card>
          ...
          <Tabs defaultValue="context">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {caseStudy.culturalContext.values.map((value, index) => (
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="space-y-1">
                      {caseStudy.culturalContext.communication.verbal.map(
                        (style, index) => (
                          <li key={index} className="text-sm">{style}</li>
                        )
                      )}
                    </ul>
                    <h5 className="text-sm font-medium">Non-verbal</h5>
                    <ul className="space-y-1">
                      {caseStudy.culturalContext.communication.nonverbal.map(
                        (style, index) => (
                          <li key={index} className="text-sm">{style}</li>
                        )
                      )}
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {caseStudy.culturalContext.familyDynamics.map(
                      (dynamic, index) => (
                        <li key={index} className="flex items-center gap-2">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {caseStudy.culturalContext.healthBeliefs.map(
                      (belief, index) => (
                        <li key={index} className="flex items-center gap-2">
            ...
            <TabsContent value="challenge" className="space-y-4">
              <div className="mb-6">
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentChallengePoint.culturalFactors.map((factor, index) => (
              ...
              <div className="space-y-4">
                ...
                {currentChallengePoint.options.map(option => (
                  <div
                    key={option.id}
                    className="p-4 border rounded-lg hover:bg-primary/5"
                  >
                    <div className="mb-2">
