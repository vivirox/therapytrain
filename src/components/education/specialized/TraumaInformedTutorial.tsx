  ...
  const updateClientState = (changes: Partial<ClientState>) => {
    setClientState(prev => {
      ...
      Object.entries(changes).forEach(([key, value]) => {
        if (key in newState) {
          newState[key as keyof ClientState] = Math.min(
            100,
            Math.max(0, newState[key as keyof ClientState] + (value as number))
          );
        }
      });
      return newState;
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Progress value={clientState.trust} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between mb-1">
              ...
              <Progress value={clientState.safety} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between mb-1">
              ...
              <Progress value={clientState.empowerment} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between mb-1">
              ...
              <Progress value={clientState.regulation} className="h-2" />
            </div>
          </div>
        </Card>
        ...
        <Card className="col-span-2 p-6">
          <Tabs defaultValue="protocols">
            ...
            <TabsContent value="protocols" className="space-y-4">
              {scenario.safetyProtocols.map(protocol => (
                <div
                  key={protocol.name}
                ...
                >
                  ...
                  <Button
                    variant="outline"
                    onClick={() => implementSafetyProtocol(protocol)}
                  >
                    Implement Protocol
                  </Button>
                </div>
            ...
            <TabsContent value="interventions" className="space-y-4">
              {scenario.interventionOptions.map(intervention => (
                <div
                  key={intervention.id}
                  className="p-4 border rounded-lg hover:bg-primary/5"
                >
                  ...
                  <Button
                    ...
                    disabled={activeInterventions.includes(intervention.id)}
                  >
                    Apply Intervention
                  </Button>
                </div>
