...
export const CrisisInterventionTutorial: React.FC<CrisisInterventionTutorialProps> = ({
  userId,
  scenarioId,
  onComplete
}) => {
  ...
  const [currentAssessment, setCurrentAssessment] = useState<SafetyAssessment | null>(
    null
  );
  ...
  useEffect(() => {
    if (!scenario || isEmergency) return;
  ...
  const updateClientState = () => {
    ...
    setClientState(prev => ({
      ...
      agitation: Math.min(
        100,
        prev.agitation + (prev.stability < 40 ? 2 : -1)
      ),
      stability: Math.max(
        0,
        prev.stability + (activeResources.includes('medical') ? 1 : -1)
      ),
  ...
  const adjustVitalSign = (vital: VitalSign, stability: number): number => {
    ...
    return Math.max(
      ...
      Math.min(
        ...
        vital.value + (Math.random() * 2 - 1) * deviation
      )
    );
  };
  ...
  const checkEscalationPoints = () => {
    ...
    const escalation = scenario.escalationPoints.find(
      point =>
  ...
  const handleIntervention = (option: InterventionOption) => {
    ...
    setCriticalPoints(prev =>
      prev.map(point =>
        point.timestamp === currentTime ? { ...point, handled: true } : point
      )
    );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <Card className="col-span-2 p-6">
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {currentAssessment.vitalSigns.map(vital => (
                <div key={vital.type} className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          {scenario.interventionOptions
            ...
            .map(option => (
              <Button
                key={option.id}
                variant={option.type === 'immediate' ? 'default' : 'outline'}
                className="w-full text-left justify-start"
                onClick={() => handleIntervention(option)}
              >
                <div>
                  <div className="flex items-center gap-2">
      ...
      <Button
        ...
        disabled={currentTime < scenario.timeLimit || isEmergency}
      >
        Complete Scenario
      </Button>
    </div>
