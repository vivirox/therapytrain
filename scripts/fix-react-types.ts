  ...
  sourceFiles.forEach(sourceFile => {
    ...
    callExpressions.forEach(callExpr => {
      ...
      if (propertyAccess) {
        ...
        if (['map', 'filter', 'reduce', 'forEach', 'some', 'every'].includes(methodName)) {
              ...
              if (!param.getTypeNode()) {
                ...
                switch (eventType) {
                  case 'click':
                    ...
                    break;
                  case 'change':
                    param.setType('React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>');
                    break;
                  case 'submit':
        ...
        properties.forEach(prop => {
          ...
          if (propName.startsWith('on') && /[A-Z]/.test(propName[2] || '')) {
            ...
            if (!typeNode || typeNode.getText() === 'any') {
              ...
              switch (eventType) {
                case 'click':
                  prop.setType('(event: React.MouseEvent<HTMLElement>) => void');
                  break;
                case 'change':
                  prop.setType('(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void');
                  break;
                case 'submit':
                  prop.setType('(event: React.FormEvent<HTMLFormElement>) => void');
                  break;
                default:
