...
async function generateKeys() {
    try {
        ...
        await snarkjs.zKey.newZKey(path.join(BUILD_DIR, CIRCUIT_NAME + '.r1cs'), path.join(BUILD_DIR, PHASE1_PATH), path.join(BUILD_DIR, CIRCUIT_NAME + '_0.zkey'));
        // Contribute to phase 2 ceremony
        console.log('Contributing to phase 2 ceremony...');
        await snarkjs.zKey.contribute(path.join(BUILD_DIR, CIRCUIT_NAME + '_0.zkey'), path.join(BUILD_DIR, CIRCUIT_NAME + '_1.zkey'), 'First contribution', generateRandomEntropy());
        // Generate verification key
        console.log('Generating verification key...');
        const vKey = await snarkjs.zKey.exportVerificationKey(path.join(BUILD_DIR, CIRCUIT_NAME + '_1.zkey'));
        await fs.writeFile(path.join(PUBLIC_DIR, CIRCUIT_NAME + '_verification_key.json'), JSON.stringify(vKey, null, 2));
    }
    catch (error) {
...
async function main() {
    try {
        ...
        try {
            const solidityVerifier = await snarkjs.zKey.exportSolidityVerifier(path.join(BUILD_DIR, CIRCUIT_NAME + '_1.zkey'), { groth16: path.join(BUILD_DIR, CIRCUIT_NAME + '_verifier_template.sol') });
            await fs.writeFile(path.join(BUILD_DIR, CIRCUIT_NAME + '_verifier.sol'), solidityVerifier);
        }
        catch (error) {
        ...
        try {
            ...
            const { proof, publicSignals } = await snarkjs.groth16.fullProve(input, path.join(BUILD_DIR, CIRCUIT_NAME + '_js', CIRCUIT_NAME + '.wasm'), path.join(BUILD_DIR, CIRCUIT_NAME + '_1.zkey'));
            // Load verification key
            const vKeyJson = await fs.readFile(path.join(PUBLIC_DIR, CIRCUIT_NAME + '_verification_key.json'), 'utf-8');
            const vKey = JSON.parse(vKeyJson);
            // Verify the proof
            const verified = await snarkjs.groth16.verify(vKey, publicSignals, proof);
            console.log('Example proof verification:', verified);
            // Save example proof and signals
            await fs.writeFile(path.join(PUBLIC_DIR, CIRCUIT_NAME + '_example_proof.json'), JSON.stringify({ proof, publicSignals }, null, 2));
        }
        catch (error) {
